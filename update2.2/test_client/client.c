...
PST_MSG makePack(EN_MSG_TYPE type,void *buf)
{
	...
	switch(type)
	{
		case TYPE_MSG_HEAD:
			...
			strcpy(((PST_FILE_HEADER)(msg->buf))->name,((PST_FILE_HEADER)buf)->name);
			((PST_FILE_HEADER)(msg->buf))->version = ((PST_FILE_HEADER)buf)->version;
			((PST_FILE_HEADER)(msg->buf))->size = ((PST_FILE_HEADER)buf)->size;
			((PST_FILE_HEADER)(msg->buf))->type = ((PST_FILE_HEADER)buf)->type;
			((PST_FILE_HEADER)(msg->buf))->crc = ((PST_FILE_HEADER)buf)->crc;
