...
{
	CLI_PRINT("Argment less then 2\n");
	CLI_PRINT("Usage:\n");
	CLI_PRINT("%s filename\n",argv[0]);
}
...
int main(int argc,char *argv[])
{
	...
	/* 打开文件 */
	CLI_PRINT("Try open %s\n",argv[1]);
	r_fd = open(argv[1],O_RDONLY);
	if(r_fd < 0)
	{
		CLI_PRINT("file %s open fail.\n",argv[1]);
		return RET_FAIL;
	}
	
	/* 获取文件的大小 */
	if((fileSize = lseek(r_fd,0L,SEEK_END)) < 0)
	{
		CLI_PRINT("Get size error.\n");
	}
	CLI_PRINT("file size: %lu\n",fileSize);
	lseek(r_fd,0L,SEEK_SET);
	...
	while(1)
	{
		if(sendFlag == 0)
		{
			...
			strncpy(head->name,argv[1],strlen(argv[1]));
			head->type 		= TYPE_PISC;	// 文件类型,区别于消息类型
			head->size 		= fileSize;			// 乱填的数据
			//head->size 		= 11985;		// 乱填的数据
			head->version 	= 1;			// 乱填的数据
			head->crc 		= 1199419934;	// 乱填的数据
		...
		else
		{
...
#if 1
			CLI_PRINT("malloc space for reading file.\n");
			if(NULL == fileBuf)
			{
				fileBuf = malloc(fileSize);
				if(NULL == fileBuf)
				{
					CLI_PRINT("Malloc fail.\n");
				}
			}

			memset(fileBuf,0,fileSize);
			/* 读取文件 */
			unsigned long sz = read(r_fd,fileBuf,fileSize);
			if(fileSize != sz)
			{
				CLI_PRINT("read size don't match fileSize.\n");
			}
			CLI_PRINT("file size : %lu\n",fileSize);
			CLI_PRINT("read size : %lu\n",sz);

			/* 计算总包数 */
			filePkg->totalPkgNum = (fileSize % SEND_BUF_LEN> 0 ? \
									(fileSize/SEND_BUF_LEN)+1:(fileSize/SEND_BUF_LEN));
			filePkg->curPkgNum		= 1;
			for(;(filePkg->curPkgNum)<=(filePkg->totalPkgNum);(filePkg->curPkgNum)++)
			{
				memset(filePkg->content,0,SEND_BUF_LEN);
				if(filePkg->curPkgNum < filePkg->totalPkgNum)
				{
					strncpy(filePkg->content,fileBuf+(filePkg->curPkgNum-1)*SEND_BUF_LEN,SEND_BUF_LEN);
				}
				else
				{
					strncpy(filePkg->content,fileBuf+(filePkg->curPkgNum-1)*SEND_BUF_LEN,\
							fileSize-(filePkg->totalPkgNum-1)*SEND_BUF_LEN);
				}
			...
			for(;(filePkg->curPkgNum)<=(filePkg->totalPkgNum);(filePkg->curPkgNum)++)
			{
				memset(filePkg->content,'a',SEND_BUF_LEN);
				PST_MSG mess = makePack(TYPE_MSG_CONTENT,filePkg);
				CLI_PRINT("Make content package %d done..\n",filePkg->curPkgNum);
				retval = sendto(src_fd,mess,sizeof(ST_MSG),0,(struct sockaddr *)&dst_addr,addr_sz);
				if(retval < 0)
