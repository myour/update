					...
					{													
						...
						{	
							...
							if(fileBuf[fb_idx].curPkgNum == fileBuf[fb_idx].totalPkgNum)
							{
								...
								{
								 #if 1
									DEBUG("Ready to unpack %s..\n",g_sysParam->fileHead.name);
									char cmdBuf[256] = {0};
									memset(cmdBuf,0,sizeof(cmdBuf));
									sprintf(cmdBuf,"tar -zxvf %s%s -C %s",REVROOT,g_sysParam->fileHead.name,REVROOT);
									...
									/* debug 删除接收文件,防止重复接收错误 */
									delRecvFile();
									
									/* 查找解压出的脚本文件并执行 */
									memset(cmdBuf,0,sizeof(cmdBuf));
									sprintf(cmdBuf,"%s%s",REVROOT,SCRIPTNAME);
									if(-1 == system(cmdBuf))
									{
										/* 执行不成功 */
										if(-1 == system(cmdBuf))
										{
											/* 1.接收文件不正确上报错误 */
											rptRevErr("do update script err");
											/* 2.删除文件 */
											DEBUG("Delete received data.\n");									
											/* 删除 */
											delRecvFile();	
